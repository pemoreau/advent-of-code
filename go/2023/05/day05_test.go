...
func TestTransformer1(t *testing.T) {
	_, result := transformer(utils.Interval{80, 80}, utils.Interval{98, 99}, 50)
	expected := []utils.Interval{{80, 80}}
	...
	_, result = transformer(utils.Interval{100, 100}, utils.Interval{98, 99}, 50)
	expected = []utils.Interval{{100, 100}}
	if !equals(result, expected) {
		t.Errorf("Result is incorrect, got: %d, want: %d.", result, expected)
	}
}
...
func TestTransformer2(t *testing.T) {
	...
	expected = []utils.Interval{{80, 97}}
	if !equals(other, expected) {
	...
	expected = []utils.Interval{{100, 101}}
	if !equals(other, expected) {
		t.Errorf("Other is incorrect, got: %d, want: %d.", result, expected)
	}
	...
	expected = []utils.Interval{{80, 97}, {100, 110}}
	if !equals(other, expected) {
		t.Errorf("Other is incorrect, got: %d, want: %d.", result, expected)
	}
}
